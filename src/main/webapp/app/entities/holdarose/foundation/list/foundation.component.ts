...
export class FoundationComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.foundationService
      ...
      .subscribe({
        next: (res: HttpResponse<IFoundation[]>) => {
  ...
  trackId(_index: number, item: IFoundation): string {
    return item.id!;
  }

  delete(foundation: IFoundation): void {
    const modalRef = this.modalService.open(FoundationDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.foundation = foundation;
  ...
  protected onSuccess(data: IFoundation[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
